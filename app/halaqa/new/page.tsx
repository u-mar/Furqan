...
export default function NewHalaqaPage() {
  ...
  const submit = async () => {
    if (busy) return
    setError('')
    if (!name.trim()) return stop('name', tr('Give the halaqa a name.'))
    if (!memberName.trim()) return stop('memberName', tr('Add your name so the others know who you are.'))
    setBusy(true)
    try {
      ...
      errorFeedback()
      setError(errorMessage(err, tr('Could not start the halaqa.')))
      setBusy(false)
    }
  }
  ...
  return (
    <HalaqaScreen>
      <HalaqaHeader title={t('New halaqa')} />
      ...
      <input
        ...
        maxLength={50}
        placeholder={t('e.g. Family Quran')}
      ...
      <SectionLabel>{t('How long')}</SectionLabel>
      <div className="home-card overflow-hidden rounded-2xl" role="radiogroup" aria-label={t('How long')}>
        <button
          type="button"
          role="radio"
        ...
        >
          ...
          <span className="min-w-0 flex-1">
            <span className="block text-[0.9375rem] font-medium">{t('Every day')}</span>
            <span className="mt-px block text-[0.78125rem] text-[var(--home-muted)]">{t('Keeps going, no end date')}</span>
          </span>
          <Radio on={!setTime} />
        </button>
        ...
        >
          ...
          <span className="min-w-0 flex-1">
            <span className="block text-[0.9375rem] font-medium">{t('For a set time')}</span>
            <span className="mt-px block text-[0.78125rem] text-[var(--home-muted)]">{t('Ramadan, 30 days, 40 days')}</span>
          </span>
          <Radio on={setTime} />
        </button>
        {setTime ? (
          <div className="qari-enter pb-3.5 pl-14 pr-3.5 pt-0.5">
            ...
            {lastDay ? (
              <p className="mt-2.5 text-[0.78125rem] text-[var(--home-muted)]">{t('Ends')} {shortDay(lastDay)}</p>
            ) : null}
          </div>
        ) : null}
      </div>
      ...
      <div className="home-card overflow-hidden rounded-2xl">
        <label className="set-row cursor-pointer" style={{ paddingBlock: 9 }}>
          ...
          <span className="min-w-0 flex-1">
            <span className="block text-[0.9375rem] font-medium">{t('Khatmah together')}</span>
            <span className="mt-px block text-[0.78125rem] text-[var(--home-muted)]">{t('Split the 30 juz between members')}</span>
          </span>
          <Switch checked={khatmah} onChange={setKhatmah} label={t('Khatmah together')} />
        </label>
        {khatmah ? (
          <div className="qari-enter">
            ...
            <label className="set-row">
              ...
              <input
                ...
                aria-label={t('Finish the khatmah by')}
                className={cn(
      ...
      <ActionButton busy={busy} onClick={() => void submit()} className="mt-6">
        {busy ? t('Creating…') : t('Create halaqa')}
      </ActionButton>
      <p className="mt-3 text-center text-[0.78125rem] text-[var(--home-muted)]">{t('You can change all of this later.')}</p>
    </HalaqaScreen>
  )
}
