...
export default function MushafTranslationView({
  ...
  return (
    <div className="relative pb-16">
            ...
            <article key={row.verse_key} className="space-y-3">
              {showHeader && (
                <div className="flex items-center justify-between px-1 pb-1 pt-2">
                  ...
                  <span className="text-sm text-stone-400">Part {part}</span>
                </div>
              )}
              <p
              ...
              >
                {row.text_uthmani}
                <span
                  className="mx-1 inline-flex h-7 min-w-[1.75rem] items-center justify-center rounded-full border border-stone-600 px-1 text-[0.65rem] font-medium text-stone-300"
              ...
              <div className="rounded-2xl bg-[#1c1c1e] px-4 py-3.5">
                <p className="text-left text-[15px] leading-relaxed text-stone-100">
                  <span className="text-stone-400">({num})</span>{' '}
      ...
      <div
        className={cn(
          'pointer-events-none fixed bottom-24 right-4 z-10',
          'rounded-full border border-stone-600 bg-[#141414]/90 px-3 py-1 text-xs text-stone-400'
        )}
      >
        {page}
      </div>
