...
export default function SurahSearchModal({
  ...
  const isAyahQuery = ARABIC_RE.test(query)
  ...
  // The full Quran text is only needed for ayah search, but with no tab to
  // signal intent up front, it has to be ready as soon as the modal opens —
  // cached after the first time, so later opens cost nothing.
  useEffect(() => {
  ...
  const ayahResults = useMemo<AyahSearchResult[]>(
    () => (verses ? searchAyahs(verses, query) : []),
    [verses, query]
  )
        ...
        <div className="flex items-center gap-2 border-b border-white/10 px-3 py-3 pt-[max(0.75rem,env(safe-area-inset-top))] lg:rounded-t-2xl lg:pt-3">
          <div className="flex min-w-0 flex-1 items-center gap-2 rounded-xl bg-[#1a1a1a] px-3 py-2.5">
            ...
            <input
              ref={inputRef}
              type="search"
              dir={isAyahQuery ? 'rtl' : 'ltr'}
              value={query}
              onChange={(e) => setQuery(e.target.value)}
              placeholder={t('Search a surah, or type an ayah in Arabic…')}
              className={cn(
                'min-w-0 flex-1 bg-transparent text-white placeholder:text-stone-500 focus:outline-none [&::-webkit-search-cancel-button]:hidden',
                isAyahQuery ? 'amiri text-base' : 'text-sm'
              )}
              autoComplete="off"
              enterKeyHint="search"
            />
