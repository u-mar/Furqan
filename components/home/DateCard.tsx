    ...
    >
      <div className="relative flex-1 px-4 pb-3 pt-4 lg:px-5 lg:pt-5">
        <div className="relative z-10">
          ...
          <p className="mt-0.5 text-sm font-medium text-emerald-600 dark:text-emerald-400 lg:text-base">
            {gregorian || '…'}
          </p>
        </div>
        <div
          className="pointer-events-none absolute right-3 top-3 flex h-14 w-14 items-center justify-center rounded-full border border-emerald-500/20 bg-emerald-500/10"
          aria-hidden
        >
          <div className="relative h-8 w-8">
            <div className="absolute left-1/2 top-1/2 h-3 w-0.5 -translate-x-1/2 -translate-y-full origin-bottom rotate-[30deg] rounded-full bg-emerald-500/80" />
            <div className="absolute left-1/2 top-1/2 h-2 w-0.5 -translate-x-1/2 -translate-y-full rounded-full bg-[var(--app-muted)]/40" />
            <div className="absolute left-1/2 top-1/2 h-1 w-1 -translate-x-1/2 -translate-y-1/2 rounded-full bg-emerald-500" />
          </div>
        </div>
      </div>
        ...
        <div className="flex items-center justify-between gap-2">
          ...
          {location ? (
            ...
            <button
              ...
              disabled={locLoading}
              className="flex min-h-[44px] shrink-0 items-center gap-1.5 rounded-lg px-2 text-xs font-semibold text-red-500 transition-colors hover:bg-black/5 active:bg-black/10 disabled:opacity-60 dark:hover:bg-white/5"
            >
              {locLoading ? (
